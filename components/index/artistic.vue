          ...
          <ul class="cbody">
            ...
            <li class="pos"
              ...
              <span
                ...
                :key="tagIndex" class="el-tabs--border-card">{{tagItem.tag}}</span></li>
  ...
  computed: {
    getImg: function () {
      return function (imgUrl) {
        if (imgUrl.indexOf('img') === -1 && imgUrl.indexOf('w.h') === -1) {
          return imgUrl
        } else return `${imgUrl.replace('img', 'p1').replace('w.h', '')}@368w_208h_1e_1c`
      }
    }
  },
  async mounted(){
    let { status, data: {pois: {data, tabs}} } = await this.$axios.get('/search/resultsByKeywords', {
      params: { keyword: this.kind, theme: 'quality', limit: 12 }
    })
    if (status === 200) {
      this.list = data
      this.tabs = tabs
